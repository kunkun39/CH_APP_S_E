...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a style="font-size:13px" href="${pageContext.request.contextPath}/backend/dashboard.html" class="tip-bottom"><i class="icon-home"></i> Dashboard</a> <a style="font-size:13px" href="javascript:void(0);" class="current">App Info Management</a> </div>
  </div>
      ...
      <div class="span12">
          ...
          <div class="widget-content">
            ...
            <table class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th style="font-size: 12px">Filters</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>
                      App Category:<select name="categoryId" style="height: 30px;">
                ...
                <tr>
                  <td>
                      App Status:
                      <span onclick="searchApp('ALL');" style="cursor: pointer" class="badge <c:if test="${'ALL'==paging.appStatus}">badge-warning</c:if>">All</span>&nbsp;&nbsp;
                      <span onclick="searchApp('CREAETED');" style="cursor: pointer" class="badge <c:if test="${'CREAETED'==paging.appStatus}">badge-warning</c:if>">Verifying</span>&nbsp;&nbsp;
                      <span onclick="searchApp('PASSED');" style="cursor: pointer" class="badge <c:if test="${'PASSED'==paging.appStatus}">badge-warning</c:if>">Published</span>&nbsp;&nbsp;
                      <span onclick="searchApp('OFFSHELVES');" style="cursor: pointer" class="badge <c:if test="${'OFFSHELVES'==paging.appStatus}">badge-warning</c:if>">Not Used</span>&nbsp;&nbsp;
                      <span onclick="searchApp('REJECTED');" style="cursor: pointer" class="badge <c:if test="${'REJECTED'==paging.appStatus}">badge-warning</c:if>">Rejected</span>
                  </td>
                </tr>
              </tbody>
            </table>
                                ...
                                <div class="fr">
                                    ...
                                    <c:choose>
                                        <c:when test="${app.status == 'CREAETED'}">
                                            <a href="javascript:void(0);" onclick="appStatusChangeConfirm('${app.id}', 'PASSED')" class="btn btn-success btn-mini">Pass</a>
                                            <a href="javascript:void(0);" onclick="appStatusChangeConfirm('${app.id}', 'REJECTED')" class="btn btn-info btn-mini">Reject</a>
                                        </c:when>
                                        <c:when test="${app.status == 'PASSED'}">
                                            <a href="javascript:void(0);" onclick="appStatusChangeConfirm('${app.id}', 'OFFSHELVES')" class="btn btn-danger btn-mini">Not Use</a>
                                        </c:when>
                                        <c:when test="${app.status == 'OFFSHELVES'}">
                                            <a href="javascript:void(0);" onclick="appStatusChangeConfirm('${app.id}', 'PASSED')" class="btn btn-inverse btn-mini">Re-publish</a>
                                        </c:when>
                                        <c:when test="${app.status == 'REJECTED'}">
                                            <a href="javascript:void(0);" onclick="appStatusChangeConfirm('${app.id}', 'CREAETED')" class="btn btn-inverse btn-mini">Re-verfily</a>
                                        </c:when>
                                    </c:choose>
                                    </security:authorize>

                                    <a href="${fileRequestHost}${app.appKey}/${app.apkActualFileName}" target="_blank" class="btn btn-warning btn-mini">Download APK</a>
                                    <a href="${pageContext.request.contextPath}/backend/marketappform.html?marketAppId=${app.id}&current=${paging.currentPageNumber}&appName=${paging.appName}&categoryId=${paging.categoryId}&topicId=${paging.topicId}&appStatus=${paging.appStatus}" class="btn btn-primary btn-mini">Edit</a>
                                    <input type="button" value="Change History" class="btn btn-success btn-mini"  onclick="openHistoryModel('${app.id}')">
...
<div id="marketapp-dialog-confirm" title="System Confirm Dialog?" style="visibility: hidden;">
    <p>
        <span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>
        Are you sure you want to change this app status?
    </p>
</div>
