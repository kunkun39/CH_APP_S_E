    ...
    public int doStartTag() throws JspException {
        try {
            ...
            if (recommend == null || recommend.getMarketAppId() <= 0) {
                if (!subRecommend) {
                    buffer.append("<img id=\"" + imageId + "\" src=\"\" title=\"recommend position " + recommendPosition + "\"/>");
                    buffer.append("<a id=\"" + nameId + "\" class=\"name\">position " + recommendPosition + "</a>");
                } else {
                    buffer.append("<p class=\"show\"><img id=\"" + imageId + "\" src=\"\" title=\"recommend position " + recommendPosition + "\"/></p>");
                    buffer.append("<a id=\"" + nameId + "\" class=\"name\">position " + recommendPosition + "</a>");
                }
            } else {
                ...
                if (!subRecommend) {
                    ...
                    if (subString && appName.length() > 20) {
                        appName = appName.substring(0, 20) + "...";
                    }
                    buffer.append("<img id=\"" + imageId + "\" src=\"" + imageURL + "\" title=\"" + recommend.getAppFullName() + "\"/>");
