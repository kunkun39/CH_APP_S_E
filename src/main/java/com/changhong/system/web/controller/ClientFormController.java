...
public class ClientFormController extends SimpleFormController {
    ...
    protected void onBindAndValidate(HttpServletRequest request, Object command, BindException errors) throws Exception {
        ...
        if (!StringUtils.hasText(macFrom)) {
            ...
            if (tokens.length != 6 || macFrom.length() != 17) {
        ...
        if (!StringUtils.hasText(macFrom)) {
            ...
            if (tokens.length != 6 || macTo.length() != 17) {
        ...
        if (!errors.hasErrors()) {
            if (macFrom.compareTo(macTo) > 0) {
                errors.rejectValue("macTo", "client.mac.range.compare");
            }
        }
    }
