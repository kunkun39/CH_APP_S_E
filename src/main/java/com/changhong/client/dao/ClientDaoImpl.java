...
public class ClientDaoImpl extends IbatisEntityObjectDao implements ClientDao {
    ...
    public List<String> loadBoxMacSupportVips(String boxMac) {
        Map<String, Object> parameters = new HashMap<String, Object>();
        parameters.put("boxMac", boxMac);
        List<HashMap> pages = getSqlMapClientTemplate().queryForList("Client.selectBoxMacSupportVips", parameters);

        List<String> supportVips = new ArrayList<String>();
        for (HashMap page : pages) {
            Integer groupId = (Integer)page.get("group_id");
            supportVips.add(String.valueOf(groupId));
        }
        return supportVips;
    }
}
