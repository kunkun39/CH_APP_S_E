...
public class ClientServiceImpl implements ClientService, InitializingBean {
    ...
    @Value("${application.mac.address.check}")
    private boolean macAddressCheck = false;
    ...
    public String obtainBootImage(String mac){
        //把开机图片组合成JSON数据流
        JSONObject all = new JSONObject();
        all.put("host", decideWhichHost());
        all.put("boot_img", cacheService.getBootImageFileName());
        if (macAddressCheck) {
            all.put("access", clientDao.loadMacCanAccess(mac));
        } else {
            all.put("access", true);
        }
    ...
    public String obtainAppDetailsInfo(int appId, String boxMac) {
        MarketAppDTO dto = cacheService.obtainMarketAppInCache(appId);
        ...
        if (dto != null) {
            ...
            app.put(ClientInfoProperties.APP_CAN_DOWNLOAD, false);
            if(vip) {
                String groupIds = dto.getGroupIds();
                List<String> supportVips = clientDao.loadBoxMacSupportVips(boxMac);
                for (String supportVip : supportVips) {
                    if (groupIds.contains(supportVip + ",")) {
                        app.put(ClientInfoProperties.APP_CAN_DOWNLOAD, true);
                        break;
                    }
                }
            } else {
